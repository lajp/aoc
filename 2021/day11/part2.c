...
void find_adjacent(int i, int j, char (*map)[100], bool (*flashed)[100]) {
    flashed[i][j] = true;
    map[i][j] = 0;
    if(i > 0) { // UP
        if(!flashed[i-1][j] && ++map[i-1][j] > 9) {
            find_adjacent(i-1, j, map, flashed);
        }
        if(j > 0) { // UP-LEFT
            if(!flashed[i-1][j-1] && ++map[i-1][j-1] > 9) {
                find_adjacent(i-1, j-1, map, flashed);
            }
        }
    ...
    if(j > 0) { // LEFT
        if(!flashed[i][j-1] && ++map[i][j-1] > 9) {
            find_adjacent(i, j-1, map, flashed);
        }
    }
    if(j < rowl-1) { // RIGHT
        if(!flashed[i][j+1] && ++map[i][j+1] > 9) {
